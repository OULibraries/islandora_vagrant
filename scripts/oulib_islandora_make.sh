...
sudo chown -hR vagrant:www-data "$DRUPAL_HOME"/sites/all


# install islandora foundation modules and library dependencies
echo "APPLYING islandora-prod.make"
drush make --yes --no-core --contrib-destination=. "$SHARED_DIR"/configs/islandora-prod.make
